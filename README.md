...
1. ```meta.py``` — делает локальные снимки содержимого публичной папки на Диске и показывает, что изменилось между двумя запусками (новые/удалённые/изменённые файлы и папки).

2. ```YAtoken.py``` - хранит в себе OAuth-токен.

3. ```addToServer.py``` - скрипт для внесения изменений в директорию облака Яндекс.Диска (удаление/добавление/изменение файлов) посредством использования OAuth-токена.
...
Устанавливаем зависимости и открываем ```meta.py ```:
```sh
...
Запускаем скрипт ```meta.py``` и вводим limit = 35.
```sh
...
В файл ```first_screen.json``` сохраняются метаданные директории ```/practice```. Файлам присущи такие характеристики, как ```name```, ```type```, ```modified```, ```size```, ```md5```; папкам - ```name```, ```type```, ```modified```. 

Содержимое ```first_screen.json```:
...
Далее для того, чтобы второй снимок был отличен от первого, внесем изменения в директорию ```/practice```. Для этого воспользуемся скриптом ```addToServer.py```. Он представляет собой 5 операций на выбор по работе с файлами/папками, описанные в результате выполнения кода ниже. Для примера создадим папку ```/practice/new_folder```:
...
```
И переместим папку ```/test``` из ```/practice/images/test``` в ```/practice/docs/archive/test```
...
Стоит подметить, что при выполнении подобных операций редактирования важно прописывать весь путь к папке/файлу, начиная с ```/practice``` и заканчивая тем элементом, который нужно отредактировать.

Теперь возвращаемся к ```meta.py```. Запустим код еще раз с тем же параметром limit = 35.
...
При повторном запуске ```meta.py``` при изменениях в директории ```/practice``` скрипт зафиксировал изменения, произведенные нами с помощью ```addToServer.py```. Содержимое файлов ```changes.json``` и ```second_screen.json```:
...
Как видим, алгоритм, реализованный в ```meta.py``` позволяет быстро мониторить изменения на Диске, а ```addToServer.py``` позволяет вносить изменения в папки/файлы для тестирования мониторинга изменений. 
